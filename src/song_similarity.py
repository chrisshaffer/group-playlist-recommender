...
class ItemItemRecommender(object):
    ...
    def fit(self, ratings_mat):
        ...
        cos_sim = cs.fit(self.ratings_mat.T)
    ...
    def _set_neighborhoods(self):
        ...
        sort_sim = cos_sim.argsort(axis=0)

        return self.ratings_mat.iloc[:,sort_sim[:self.neighborhood_size]]
    ...
    def get_ratings_data():
        ...
        Returns:
            ...
            correspond to movies. Each element is the user's rating for that movie.
        '''
        ratings_contents = pd.read_table("data/u.data",
                                        names=["user", "movie", "rating",
                                                "timestamp"])
        ratings_as_mat = sparse.csr_matrix((ratings_contents.rating,
                                        (ratings_contents.user,
                                            ratings_contents.movie)))
    ...
    def load_movies():
        ...
        movies = pd.read_table("./data/u.item",
                            names=columns,
                            sep='|',
                            encoding='latin-1')
